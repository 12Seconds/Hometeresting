 OI_RTC = function (customCam,testPlay)
 {
     var oi = this;
     ...
     oi.chatFunction = function(){
         if(oi.isRTCDoing){
             if($("#RTCKeyChatWindow").css('opacity')==0.4){
                 ...
                 customCam.dettachEvent();
                 testPlay.dettachEvent();
                 $("#inputDefault").focus();
             }
             else{
                 $("#RTCKeyChatWindow").css('opacity','0.4');
                     ...
                     if(oi.device.hasMicrophone===true && oi.device.hasSpeakers===true){
                         ...
                         oi.connection.checkPresence(oi.MultiCreateFullRoomName, function(isRoomExists, roomid){
                             if(isRoomExists){
                                 ...
                                 oi.isRTCDoing = true;
                                 $("#RTCKeyChatWindow").css("display","block");
                             }
                             else{
                                 // 기존에 있던 방에서 나감
                                 oi.connection.leave();
             ...
             oi.connection.checkPresence(oi.joinFullRoomName, function(isRoomExists, roomid){
                if(isRoomExists){
                    ...
                    oi.connection.DetectRTC.load(function(){
                        ...
                        else if(oi.device.hasMicrophone===false && oi.device.hasSpeakers===false){ // 마이크와 스피커가 둘 다 없는경우
                            oi.connection.mediaConstraints = {
                                audio: false,
                                video: false
                            };
